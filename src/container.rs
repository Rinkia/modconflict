...
const FORMATS: &[ContainerFormat] = &[
    ...
    ContainerFormat {
        // Baldur's Gate 3 and other Larian games: .pak, but LSPK not Unreal.
        // Must come before the Unreal entry, whose sniff claims any .pak.
        name: "Larian pak",
        sniff: sniff_larian,
        read: read_larian,
    },
...
pub fn read(path: &Path) -> anyhow::Result<Option<Container>> {
    ...
    let files = read_isolated(format, path)?;
    Ok(Some(Container {
        format: format.name,
...
/// Run a third-party parser behind a panic boundary.
///
/// These crates are good, but they are parsing hostile binary input, and an
/// index-out-of-bounds deep inside one of them would otherwise take down a scan
/// of two hundred perfectly readable mods. A panic here costs exactly one
/// archive, and says so.
fn read_isolated(format: &ContainerFormat, path: &Path) -> anyhow::Result<Vec<String>> {
    let read = format.read;
    let owned = path.to_path_buf();

    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(move || read(&owned))) {
        Ok(result) => result,
        Err(_) => anyhow::bail!(
            "the {} reader panicked on this file; it is malformed or unsupported",
            format.name
        ),
    }
}
...
fn sniff_larian(head: &[u8]) -> bool {
    head.starts_with(b"LSPK")
}

fn read_larian(path: &Path) -> anyhow::Result<Vec<String>> {
    let pak = larian_formats::lspk::Lspk::from_file(path).map_err(|e| anyhow::anyhow!("{e}"))?;
    Ok(pak
        .files
        .into_iter()
...
mod tests {
    ...
    #[test]
    fn a_larian_pak_is_not_mistaken_for_an_unreal_one() {
    ...
    #[test]
    fn a_panicking_reader_costs_one_archive_not_the_scan() {
        fn boom(_: &Path) -> anyhow::Result<Vec<String>> {
            panic!("simulating a parser blowing up on hostile input")
        }
        ...
        // Silence the panic hook so the test output stays readable.
        let hook = std::panic::take_hook();
        std::panic::set_hook(Box::new(|_| {}));
        let result = read_isolated(&format, Path::new("whatever"));
